...
// Test the divergent evolution concept
...
const baseGenome = sequencer.generate("planet-y2k-moon", traits);
...
const divergenceRate = 0.05; // 5% variation

for (let i = 0; i < 8; i++) {
    const variantSeed = `planet-y2k-moon-microbe-${i}`;
    const variant = sequencer.generate(variantSeed, traits);
...
for (let i = 0; i < 4; i++) {
    ...
    const variant = sequencer.generate(`planet-y2k-moon-flora-${i}`, floraTraits);
    floraVariants.push({
        id: `F-${i}`,
