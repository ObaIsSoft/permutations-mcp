...
import { StarType, DesignPhilosophy } from "./genome/types.js";
...
export interface StarEntry {
    type: StarType;
    description: string;
    /** Philosophies that never use this star type */
    forbiddenFor: {
        philosophies?: DesignPhilosophy[];
        entropyCeiling?: number;
        entropyFloor?: number;
        requiresPhysics?: string[];  // at least one of these must be active
        requiresVideo?: boolean;     // only eligible when ch20.hasVideo === true
    };
    /** headline scaling relative to star visual weight */
    headlineScale: "down" | "up" | "normal";
    /** supporting text gets ghost_text treatment to avoid competing with star */
    ghostSupportingText: boolean;
    /** CTA placement relative to star's visual exit point */
    ctaAnchor: "bottom_right_of_star" | "below_center" | "overlay_bottom" | "adjacent_right";
    /**
     * Which CSS custom property names drive the star's layout dimensions.
     * These are declared in :root by generateStarCSS — the values come from
     * genome variables, not hardcoded numbers.
     */
    dimensionVars: {
        width: string;   // CSS clamp or var() expression using genome vars
        height?: string;
    };
}
...
export function getStarEntry(type: StarType): StarEntry {
 ...
 */
export function generateStarCSS(star: StarEntry): string {
    if (star.type === "none") {
    ...
    if (star.type === "oversized_phrase" || star.type === "kinetic_type") {
        ...
        lines.push(
            ...
            `  background:`,
            `    radial-gradient(ellipse at 20% 50%, color-mix(in oklch, var(--color-primary) 40%, transparent) 0%, transparent 50%),`,
            `    radial-gradient(ellipse at 80% 20%, color-mix(in oklch, var(--color-accent) 35%, transparent) 0%, transparent 50%),`,
            `    radial-gradient(ellipse at 50% 80%, color-mix(in oklch, var(--color-primary) 25%, transparent) 0%, transparent 60%);`,
            `  background-color: var(--color-bg);`,
            `  animation: hero-gradient-drift var(--duration-ambient, 8s) ease-in-out infinite alternate;`,
            `}`,
        ...
        lines.push(
            ``,
            `.hero-star-noise { position: absolute; inset: 0; z-index: 0; }`,
            `.hero-star-noise canvas { width: 100%; height: 100%; display: block; }`,
        );
    } else if (star.type === "color_field") {
        lines.push(
            ``,
            `.hero-star-color-field {`,
            `  position: absolute; inset: 0; z-index: 0;`,
        ...
        lines.push(
            ...
            `  max-height: 100vh;`,
            `}`,
            `.hero-star-mosaic .mosaic-item:first-child { grid-row: 1 / 3; }`,
            `.hero-star-mosaic .mosaic-item:last-child  { grid-column: 2; grid-row: 2 / 4; }`,
            `.hero-star-mosaic .mosaic-item { overflow: hidden; border-radius: var(--radius-md); }`,
            `.hero-star-mosaic .mosaic-item img { width: 100%; height: 100%; object-fit: cover; }`,
        );
    } else if (star.type === "video_loop") {
        lines.push(
            ``,
            `.hero-star-video { position: absolute; inset: 0; z-index: 0; }`,
            `.hero-star-video video { width: 100%; height: 100%; object-fit: cover; }`,
        );
    } else if (star.type === "signature_image") {
        lines.push(
            ``,
            `.hero-star-image { position: absolute; inset: 0; z-index: 0; }`,
            `.hero-star-image img { width: 100%; height: 100%; object-fit: cover; }`,
        );
        ...
        lines.push(
            ...
            `  animation: svg-draw var(--duration-slow, 2s) ease-out forwards;`,
            `}`,
