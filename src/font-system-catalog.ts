...
import { DesignPhilosophy, FontStrategy, TypeCharge } from "./genome/types.js";
 ...
 */
export function deriveFontCount(
    ...
    if (philosophy === "minimalist") return 1;
    if (philosophy === "swiss_grid") return entropy < 0.50 ? 1 : 2;
    if (philosophy === "technical") return physics === "none" ? 1 : 2;
    if (philosophy === "chaotic") return 3;
    if (philosophy === "expressive") return 3;

    // Standard and editorial are entropy-dependent
    if (entropy < 0.30) return 1;
    if (entropy > 0.65) return 3;

    // editorial leans toward 2-3
    if (philosophy === "editorial") return pool.getFloat(poolOffset) > 0.40 ? 3 : 2;

    // brand_heavy: almost always 2
    if (philosophy === "brand_heavy") return pool.getFloat(poolOffset) > 0.70 ? 3 : 2;

    // Default: 2
    return 2;
}

// ── Font strategy catalog ─────────────────────────────────────────────────────

interface FontStrategyEntry {
    strategy: FontStrategy;
    fontCount: 1 | 2 | 3;
    anchorCharge: TypeCharge | TypeCharge[];    // required charge for display font
    bodyCharge?: TypeCharge | TypeCharge[];     // required charge for body (count ≥ 2)
    accentCharge?: TypeCharge | TypeCharge[];   // required charge for accent (count = 3)
    description: string;
    forbiddenFor: {
        philosophies?: DesignPhilosophy[];
        entropyCeiling?: number;    // not valid above this entropy
        entropyFloor?: number;      // not valid below this entropy
    };
}

export const FONT_STRATEGY_CATALOG: FontStrategyEntry[] = [
    // ── 1-font strategies ──
    {
...
export function selectFontStrategy(
    fontCount: 1 | 2 | 3,
    philosophy: DesignPhilosophy,
    entropy: number,
    pool: EntropyPool,
    ...
    const eligible = FONT_STRATEGY_CATALOG.filter(entry => {
        if (entry.fontCount !== fontCount) return false;
        if (entry.forbiddenFor.philosophies?.includes(philosophy)) return false;
        if (entry.forbiddenFor.entropyCeiling !== undefined && entropy > entry.forbiddenFor.entropyCeiling) return false;
        if (entry.forbiddenFor.entropyFloor !== undefined && entropy < entry.forbiddenFor.entropyFloor) return false;
        return true;
    });
 ...
 */
export function getFontChargesForStrategy(strategy: FontStrategy): {
    anchor: TypeCharge | TypeCharge[];
    body: TypeCharge | TypeCharge[] | undefined;
    accent: TypeCharge | TypeCharge[] | undefined;
} {
 ...
 */
export function generateFontSystemCSS(
    fontCount: 1 | 2 | 3,
    anchorFamily: string,
    bodyFamily: string | null,
    accentFamily: string | null
): string {
...
export function generateFontPreloads(
    fontCount: 1 | 2 | 3,
    anchorImportUrl: string,
    bodyImportUrl: string | null,
    accentImportUrl: string | null
): string[] {
