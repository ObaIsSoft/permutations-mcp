...
interface LLMResponse {
    ...
    copy?: {
        ...
        companyName?: string;        // Product/company name from intent
        features?: { title: string; description: string }[];  // 3 key features
        stats?: { label: string; value: string }[];           // 3 key stats
        testimonial?: string;        // Customer quote
        faq?: { question: string; answer: string }[];         // 2 FAQ items
    };
}
