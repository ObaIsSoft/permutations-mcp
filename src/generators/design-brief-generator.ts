...
export class DesignBriefGenerator {
    generate(genome: DesignGenome): DesignBrief {
        ...
        const summary = `A ${c.ch2_rhythm.density} design system for ${genome.sectorContext.primary} ` +
                       `with ${c.ch26_copy_intelligence?.emotionalRegister || "professional"} tone. ` +
    ...
    private describeContent(c: DesignGenome["chromosomes"]): string {
        const ci = c.ch26_copy_intelligence;
...
- Industry Terms: ${c.ch26_copy_intelligence?.industryTerminology.slice(0, 5).join(", ") || "N/A"}
- Emotional Register: ${c.ch26_copy_intelligence?.emotionalRegister || "N/A"}
- Formality Level: ${Math.round((c.ch26_copy_intelligence?.formalityLevel || 0.5) * 100)}%
