...
export interface WriteOptions {
    ...
    /** Emit package.json alongside component files (default: true) */
    emitPackageJson?: boolean;
    /** Emit USAGE.md explaining file roles and load order (default: true) */
    emitUsageMd?: boolean;
}
...
export class DesignFileWriter {
    ...
    async writeDesignSystem(
        ...
        outputs: {
            ...
            architecture?: string;
            tokens?: string;
            /** CSS-only token file — write as tokens.css */
            tokensCss?: string;
            /** Tailwind config — write as tailwind.config.js */
        ...
        try {
            ...
            // Main CSS (full genome stylesheet)
            ...
            // Separate token files — tokensCss (CSS :root only) and tailwindConfig (JS only)
            if (outputs.tokensCss) {
                writes.push(this.writeFile(
                    path.join(options.baseDir, "styles", "tokens.css"),
                    outputs.tokensCss,
                    result
                ));
            }

            if (outputs.tailwindConfig) {
                writes.push(this.writeFile(
                    path.join(options.baseDir, "tailwind.config.js"),
                    outputs.tailwindConfig,
                    result
                ));
            }
            ...
            // Write full genome (all chromosomes — not a thin manifest)
            writes.push(this.writeFile(
            ...
            // Emit package.json listing exact runtime dependencies
            if (options.emitPackageJson !== false) {
                const deps = this.buildDependencies(options.framework, outputs);
                writes.push(this.writeFile(
            ...
            // Emit USAGE.md explaining what each file is and how to load it
            if (options.emitUsageMd !== false) {
                writes.push(this.writeFile(
                    path.join(options.baseDir, "USAGE.md"),
                    this.generateUsageMd(genome, options, outputs),
                    result
                ));
            }
...
<body>
    ...
    private buildDependencies(
        framework: WriteOptions["framework"],
        outputs: { components?: string; webgl?: string; animations?: string }
    ): Record<string, unknown> {
        const hasMotion = !!outputs.animations || !!outputs.components;
        const hasWebGL = !!outputs.webgl;
        ...
        if (hasMotion && framework === "react") {
            deps["framer-motion"] = "^11.0.0";
        }

        if (hasWebGL && framework === "react") {
            deps["@react-three/fiber"] = "^8.0.0";
            deps["three"] = "^0.165.0";
            devDeps["@types/three"] = "^0.165.0";
        }

        return {
            name: "permutations-design-system",
            version: "0.0.1",
            private: true,
            description: "Generated by Permutations MCP — do not edit manually",
            dependencies: deps,
    ...
    private generateUsageMd(
        ...
        const hash = genome.dnaHash.slice(0, 12);
        const tier = (genome as any).tier ?? "unknown";
        const hasReact = options.framework === "react" && !!outputs.components;
        const hasTailwind = !!outputs.tailwindConfig;

        const lines: string[] = [
            `# Permutations Design System`,
            ``,
            `Genome: \`${hash}\` | Tier: \`${tier}\` | Framework: \`${options.framework}\``,
            ``,
            `> **Scaffold notice**: Component files contain placeholder structures.`,
            `> Wire up props, handlers, and aria attributes before shipping to production.`,
            ``,
            `## File Roles`,
            ``,
            `| File | Type | Requires |`,
            `|------|------|----------|`,
            `| \`styles/genome.css\` | CSS only | Link in \`<head>\` |`,
        ];

        if (outputs.tokensCss) {
            lines.push(`| \`styles/tokens.css\` | CSS \`:root\` vars | Link before genome.css |`);
        }
        if (outputs.tailwindConfig) {
            lines.push(`| \`tailwind.config.js\` | Tailwind extension | Bundler + Tailwind CSS |`);
        }
        if (outputs.components) {
            lines.push(`| \`components/DesignSystem.tsx\` | React/TS scaffold | npm install + bundler |`);
        }
        if (outputs.animations) {
            lines.push(`| \`lib/animations.ts\` | Framer Motion config | framer-motion package |`);
        }
        if (outputs.webgl) {
            lines.push(`| \`components/Procedural3D.tsx\` | Three.js/R3F | @react-three/fiber, three |`);
        }
        if (outputs.fx) {
            lines.push(`| \`styles/atmosphere.css\` | CSS FX layer | Add \`.fx-atmosphere\` class to hero element |`);
        }
        if (outputs.svg) {
            lines.push(`| \`assets/biomarker.svg\` | Static SVG | Embed directly in HTML |`);
        }
        lines.push(`| \`genome.json\` | Full chromosome data | Read by runtime or CI |`);

        lines.push(``, `## Load Order`, ``);
        lines.push(`\`\`\`html`);
        lines.push(`<!-- 1. Fonts (from genome.json ch3/ch4) -->`);
        lines.push(`<link rel="preconnect" href="https://fonts.googleapis.com">`);
        lines.push(`<!-- 2. Tokens (CSS vars only) -->`);
        if (outputs.tokensCss) lines.push(`<link rel="stylesheet" href="styles/tokens.css">`);
        lines.push(`<!-- 3. Full genome stylesheet -->`);
        lines.push(`<link rel="stylesheet" href="styles/genome.css">`);
        if (outputs.fx) lines.push(`<!-- 4. FX layer — apply .fx-atmosphere to hero section -->`);
        if (outputs.fx) lines.push(`<link rel="stylesheet" href="styles/atmosphere.css">`);
        lines.push(`\`\`\``);

        lines.push(``, `## Quick Start`, ``);
        lines.push(`\`\`\`bash`);
        lines.push(`npm install`);
        if (hasReact) lines.push(`# Then import components:`);
        if (hasReact) lines.push(`# import { Button } from './components/DesignSystem';`);
        lines.push(`\`\`\``);

        if (hasTailwind) {
            lines.push(``, `## Tailwind Setup`, ``);
            lines.push(`The generated \`tailwind.config.js\` extends your Tailwind theme with genome values.`);
            lines.push(`Merge it with your existing config — do not replace it.`);
        }

        lines.push(``, `## MCP Registration`, ``);
        lines.push(`To use Permutations in Claude Code / Cursor / Windsurf, add to your \`.mcp.json\`:`);
        lines.push(``);
        lines.push(`\`\`\`json`);
        lines.push(`{`);
        lines.push(`  "mcpServers": {`);
        lines.push(`    "permutations": {`);
        lines.push(`      "command": "node",`);
        lines.push(`      "args": ["/path/to/permutations/dist/server.js"],`);
        lines.push(`      "env": {`);
        lines.push(`        "GROQ_API_KEY": "your-key-here"`);
        lines.push(`      }`);
        lines.push(`    }`);
        lines.push(`  }`);
        lines.push(`}`);
        lines.push(`\`\`\``);
        lines.push(``);
        lines.push(`> Note: \`.mcp.json\` is infrastructure config owned by you — the tool does not generate it`);
        lines.push(`> because it doesn't know your install path or which API keys you're using.`);
...
${spec.props.map(p => `  ${p.name}${p.required ? "" : "?"}: ${p.type};`).join("\n")}
        ...
        // aria props as comments — aria-disabled={aria-disabled} is invalid JSX (circular ref)
        const ariaComment = spec.accessibility.ariaProps.length > 0
            ? `      {/* aria: ${spec.accessibility.ariaProps.join(', ')} — wire to props when implementing */}`
            : '';

        return `import React from 'react';${motion !== "none" ? "\nimport { motion } from 'framer-motion';" : ""}

// SCAFFOLD — wire up props/logic before shipping to production
// aria: ${spec.accessibility.ariaProps.join(', ') || 'none'} | keyboard: ${spec.accessibility.keyboard.join(', ') || 'none'}
${typescript ? "\n" + propsInterface : ""}
...
export function ${spec.name}({ ${propsDestructuring} }${typescript ? `: ${spec.name}Props` : ""}) {
  return (
    ...
    >
${ariaComment}
      {/* ${spec.name} — implement based on design requirements */}
    ${motion !== "none" ? "</motion.div>" : "</div>"}
  );
}
`;
