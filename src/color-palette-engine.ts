...
import { HarmonyRule, ColorPalette, PaletteColor, DesignPhilosophy } from "./genome/types.js";
...
const HUE_NAMES: Array<{ hMin: number; hMax: number; lDark: string; lMid: string; lLight: string }> = [
...
function nameFromOklch(l: number, c: number, h: number): string {
    // Low chroma = neutral
    if (c < 0.04) {
    ...
    if (l < 0.35) return entry.lDark;
    if (l < 0.65) return entry.lMid;
    return entry.lLight;
}
...
function buildMonochromatic(baseH: number, baseC: number, _pool: EntropyPool, offset: number): PaletteColor[] {
    const lightnessSteps = [0.15, 0.35, 0.55, 0.75, 0.90];
    const roles: PaletteColor["role"][] = ["background", "primary", "secondary", "surface", "neutral"];
    const hints = [["page bg"], ["headlines, CTAs"], ["subheads, icons"], ["card surfaces"], ["borders, labels"]];
    return lightnessSteps.map((l, i) =>
        makeColor(l, baseC * (1 - i * 0.12), baseH, roles[i], hints[i])
    );
}

function buildAnalogousTight(baseH: number, baseC: number, _pool: EntropyPool, _offset: number): PaletteColor[] {
    ...
    const roles: PaletteColor["role"][] = ["primary", "secondary", "accent", "surface", "background"];
...
function buildAnalogousWide(baseH: number, baseC: number, _pool: EntropyPool, _offset: number): PaletteColor[] {
    ...
    const roles: PaletteColor["role"][] = ["primary", "secondary", "accent", "neutral", "background"];
    const hints = [["CTAs"], ["subheads"], ["highlights"], ["muted text"], ["page bg"]];
    return hues.map((h, i) => makeColor(lightnesses[i], sats[i], h, roles[i], hints[i]));
}

function buildComplementary(baseH: number, baseC: number, _pool: EntropyPool, _offset: number): PaletteColor[] {
...
function buildSplitComplementary(baseH: number, baseC: number, _pool: EntropyPool, _offset: number): PaletteColor[] {
    return [
        makeColor(0.45, baseC, baseH, "primary", ["CTAs, headlines"]),
...
function buildTriadic(baseH: number, baseC: number, _pool: EntropyPool, _offset: number): PaletteColor[] {
    return [
        makeColor(0.45, baseC, baseH, "primary", ["CTAs, headlines"]),
...
function buildTetradic(baseH: number, baseC: number, pool: EntropyPool, offset: number): PaletteColor[] {
    const colors = [
        makeColor(0.45, baseC, baseH, "primary", ["CTAs, headlines"]),
...
function buildDoubleComplementary(baseH: number, baseC: number, _pool: EntropyPool, _offset: number): PaletteColor[] {
    return [
...
function buildNeutralDominant(baseH: number, baseC: number, _pool: EntropyPool, _offset: number): PaletteColor[] {
...
function buildDarkAccent(baseH: number, baseC: number, _pool: EntropyPool, _offset: number): PaletteColor[] {
    return [
...
function buildPastelSystem(baseH: number, baseC: number, _pool: EntropyPool, _offset: number): PaletteColor[] {
    return [
...
function buildNeonDark(baseH: number, baseC: number, _pool: EntropyPool, _offset: number): PaletteColor[] {
    return [
...
const HARMONY_BUILDERS: Record<HarmonyRule, (baseH: number, baseC: number, pool: EntropyPool, offset: number) => PaletteColor[]> = {
...
export function selectHarmonyRule(
    entropy: number,
    isDark: boolean,
    physics: string,
    sector: string,
    ...
    const eligible = all.filter(rule => {
        ...
        if (rule === "monochromatic" && entropy > 0.70) return false;           // too safe for chaotic
        if (rule === "triadic" && entropy < 0.40) return false;                 // too bold for conservative
        if (rule === "tetradic" && entropy < 0.50) return false;
        if (rule === "double_complementary" && entropy < 0.60) return false;
        if (rule === "neon_dark" && !["glitch", "particle"].includes(physics)) return false;
        if (rule === "neon_dark" && entropy < 0.55) return false;
        if (rule === "dark_accent" && !isDark && entropy < 0.40) return false;
        ...
        if (rule === "pastel_system" && !["beauty_fashion", "education", "healthcare"].includes(sector)) {
            if (entropy > 0.55) return false; // bold genomes don't do pastels
        }
        // Philosophy gates
        if (rule === "neutral_dominant" && philosophy === "expressive") return false;
        if (rule === "neutral_dominant" && philosophy === "chaotic") return false;
        if (rule === "monochromatic" && philosophy === "chaotic") return false;
        return true;
    });
...
export function generatePalette(
    primaryHue: number,       // from ch5_color_primary.hue (0-360)
    primarySat: number,       // 0-1
    entropy: number,          // from ch12_signature.entropy
    isDark: boolean,
    physics: string,
    sector: string,
    philosophy: DesignPhilosophy,
...
export function generatePaletteCSS(palette: ColorPalette): string {
    ...
    const oklchVars = palette.colors.map((c, i) =>
        `  --palette-${i + 1}: oklch(${c.oklch.l.toFixed(3)} ${c.oklch.c.toFixed(3)} ${c.oklch.h.toFixed(1)}); /* ${c.name} — ${c.role} */`
    ).join("\n");
