    ...
    async writeDesignSystem(genome, outputs, options) {
        ...
        try {
            ...
            if (outputs.tokensCss) {
                writes.push(this.writeFile(path.join(options.baseDir, "styles", "tokens.css"), outputs.tokensCss, result));
            }
            if (outputs.tailwindConfig) {
                writes.push(this.writeFile(path.join(options.baseDir, "tailwind.config.js"), outputs.tailwindConfig, result));
            }
            ...
            if (options.emitPackageJson !== false) {
                ...
                writes.push(this.writeFile(path.join(options.baseDir, "package.json"), JSON.stringify(deps, null, 2), result));
            }
            // Emit USAGE.md explaining what each file is and how to load it
            if (options.emitUsageMd !== false) {
                writes.push(this.writeFile(path.join(options.baseDir, "USAGE.md"), this.generateUsageMd(genome, options, outputs), result));
            }
...
<body>
    ...
    buildDependencies(framework, outputs) {
    ...
    generateUsageMd(genome, options, outputs) {
        const hash = genome.dnaHash.slice(0, 12);
        const tier = genome.tier ?? "unknown";
        ...
        if (outputs.tokensCss)
            lines.push(`<link rel="stylesheet" href="styles/tokens.css">`);
        lines.push(`<!-- 3. Full genome stylesheet -->`);
        lines.push(`<link rel="stylesheet" href="styles/genome.css">`);
        if (outputs.fx)
            lines.push(`<!-- 4. FX layer — apply .fx-atmosphere to hero section -->`);
        if (outputs.fx)
            lines.push(`<link rel="stylesheet" href="styles/atmosphere.css">`);
        lines.push(`\`\`\``);
        lines.push(``, `## Quick Start`, ``);
        lines.push(`\`\`\`bash`);
        lines.push(`npm install`);
        if (hasReact)
            lines.push(`# Then import components:`);
        if (hasReact)
            lines.push(`# import { Button } from './components/DesignSystem';`);
        lines.push(`\`\`\``);
        if (hasTailwind) {
