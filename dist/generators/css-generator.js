...
export class CSSGenerator {
    generate(genome, format = "tailwind") {
        ...
        if (format === "css")
            return this.generateCSSVariables(genome);
        if (format === "scss")
            return this.generateSCSS(genome);
    ...
    generateTailwindConfig(genome) {
        ...
        const forbiddenPatterns = genome.constraints.forbiddenPatterns.map((p) => `// FORBIDDEN: ${p}`).join('\n');
        const requiredPatterns = genome.constraints.requiredPatterns.map((p) => `// REQUIRED: ${p}`).join('\n');
        const motionEasing = ch.ch8_motion.physics === "spring" ? "cubic-bezier(0.34, 1.56, 0.64, 1)" :
