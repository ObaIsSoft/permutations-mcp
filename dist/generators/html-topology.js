...
export class HTMLTopologyGenerator {
    ...
    applyConstraints(sections, forbidden, required, traits) {
    ...
    buildReasoning(topology, traits) {
        const reasons = [];
        if (topology === "flat")
            reasons.push(`High density ${traits.informationDensity.toFixed(2)} → flat modular cards`);
        if (topology === "deep")
            reasons.push(`Low urgency ${traits.temporalUrgency.toFixed(2)} + long-form → deep reading topology`);
        if (topology === "radial")
            reasons.push(`High spatial ${traits.spatialDependency.toFixed(2)} → orbital/hub layout`);
        if (topology === "graph")
            reasons.push(`Playful ${traits.playfulness.toFixed(2)} + spatial → network/graph structure`);
