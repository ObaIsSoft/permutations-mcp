 ...
 */
export const DEPTH_TECHNIQUE_CATALOG = [
    // ── Stratum ──────────────────────────────────────────────────────────────
...
export const PHILOSOPHY_TECHNIQUES = {
...
export function selectDepthTechniques(ctx) {
    ...
    if (ctx.entropy > 0.40 && ctx.designPhilosophy === "editorial")
        base.push("image_cutout_depth");
    if (ctx.material === "glass")
        base.push("backdrop_filter_blur", "specular_highlight");
    if (ctx.material === "clay" || ctx.material === "neumorphic")
        base.push("inset_shadow");
    if (["spring", "wave", "magnetic"].includes(ctx.physics) && ctx.philosophy === "immersive") {
        base.push("perspective_section");
    }
    if (ctx.philosophy !== "flat")
        base.push("noise_texture_surface");
    ...
    return base.filter(id => {
        ...
        if (!tech)
            return false;
        if (!tech.validIn.includes(ctx.philosophy))
            return false;
        const r = tech.requires;
        if (!r)
            return true;
        if (r.isDark !== undefined && r.isDark !== ctx.isDark)
            return false;
        if (r.minEntropy !== undefined && ctx.entropy < r.minEntropy)
            return false;
        if (r.minNoiseLevel !== undefined && ctx.noiseLevel < r.minNoiseLevel)
            return false;
        if (r.physics && !r.physics.includes(ctx.physics))
            return false;
        if (r.material && !r.material.includes(ctx.material))
            return false;
        return true;
    });
}
 ...
 */
export function generateDepthCSS(ctx) {
    const active = selectDepthTechniques(ctx);
    if (active.length === 0)
        return `/* depth: ${ctx.philosophy} — no techniques active */`;
    const blocks = [
 ...
 */
function generateTechniqueCSS(id) {
    switch (id) {
