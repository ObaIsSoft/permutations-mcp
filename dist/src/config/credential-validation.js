...
export const OPTIONAL_CREDENTIALS = [
    {
 ...
 */
export async function validateCredentialsOnStartup() {
    const errors = [];
    const warnings = [];
    ...
    for (const cred of REQUIRED_CREDENTIALS) {
        ...
        if (!value || value.trim() === '') {
            errors.push(`[Startup] Missing required credential: ${cred.name} (${cred.envVar})\n` +
        ...
        if (cred.validate) {
            ...
            if (!validation.valid) {
                errors.push(`[Startup] Invalid credential: ${cred.name}\n` +
    ...
    for (const cred of OPTIONAL_CREDENTIALS) {
        const value = process.env[cred.envVar];
        if (!value || value.trim() === '') {
            warnings.push(`[Startup] Optional credential not set: ${cred.name} (${cred.envVar})\n` +
                `  ├─ Description: ${cred.description}\n` +
 ...
 */
export function printCredentialValidation(result) {
